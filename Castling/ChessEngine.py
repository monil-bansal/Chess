...
class GameState():
	def __init__(self):
		...
		self.castleRightsLog = [CastleRights(self.currentCastlingRights.wks, self.currentCastlingRights.wqs, #correct way 
	...
	def makeMove(self, move):
		...
		if move.isPawnPromotion:
			self.board[move.endRow][move.endCol] = move.pieceMoved[0] + 'Q' # hardcoding Q for now. will include options later.
	...
	def undoMove(self): 
		...
		self.currentCastlingRights =  self.castleRightsLog[-1] # update current castling right
	...
	def getPawnMoves(self, r, c, moves):
		if self.whiteToMove and self.board[r][c][0] == 'w': # WHITE PAWN MOVES
			...
			if c-1 >= 0:
				if self.board[r-1][c-1][0] == 'b':  #  enemy pice to capture to the left
					moves.append(Move((r, c), (r-1, c-1), self.board))
				if self.enPassantPossible == (r-1, c-1):
					moves.append(Move((r, c), (r-1, c-1), self.board, isEnPassantMove =  True))
			if c+1 < len(self.board):
				if self.board[r-1][c+1][0] == 'b': # enemy pice to capture to the right
					moves.append(Move((r, c), (r-1, c+1), self.board))
				if self.enPassantPossible == (r-1, c+1):
					moves.append(Move((r, c), (r-1, c+1), self.board, isEnPassantMove =  True))
